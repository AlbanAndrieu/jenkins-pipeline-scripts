...
class Sonar implements Serializable {
     ...
     */
    @NonCPS
  static get_project_name(jobName, defaultProject='nabla') {
     ...
     */
    @NonCPS
  static get_repository_name(jobName) {
    def tokenized_job_name = jobName.split('/')
    if (tokenized_job_name.size() == 2) {
  ...
  static def List<String> diffCommitsForRev(Map<String,String> config) {
    try {
      ...
      echo 'Error: There were errors to connecting to sonar. ' + exc // but we do not fail the whole build because of that
        }
  }
  ...
  static def List<String> filesForRev(Map<String,String> config, String revision) {
    ...
    @NonCPS
  static getSonarInclusions(Map vars, Closure body=null) {
    ...
    @NonCPS
  static filesChanged(Map vars, Closure body=null) {
  ...
  static sonarRestCall(def apiUrl, def login, def password, def method, def query='') {
