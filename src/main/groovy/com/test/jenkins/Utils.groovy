...
class Utils {
  ...
  @NonCPS static entries(def m) { m.collect { k, v -> [k, v] } }
  ...
  @NonCPS static nodeRootDir(def nodeName) {
  ...
  @NonCPS static stashRestCall(def apiUrl, def login, def password, def method, def query='') {
  ...
  @NonCPS static callURL(def urlString) {
    Utils.callURL(urlString, 'GET')
  }

  @NonCPS static callURL(def urlString, def method) {
    Utils.callURL(urlString, '', method)
  }

  @NonCPS static jsonCallURL(def urlString, def query, def method, def headers=null) {
    headers = headers ?: [:]
  ...
  @NonCPS static callURL(def urlString, def queryString, def method, def headers=null) {
    if (queryString) {
      if (method == 'GET') {
