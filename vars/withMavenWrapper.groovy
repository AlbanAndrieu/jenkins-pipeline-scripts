...
def call(Map vars, Closure body=null) {
    ...
    echo "Maven DEFAULT OPTS have been specified: ${vars.MAVEN_OPTS} - ${vars.CLEAN_RUN}/${vars.DRY_RUN}/${vars.DEBUG_RUN} - ${vars.SONAR_INSTANCE}"
        ...
        tee("${vars.shellOutputFile}") {
            ...
            //withConfigFileProviderWrapper(vars) {
                ...
                vars.mavenGoals += " -Dmaven.repo.local=./.repository "
				        ...
				        echo "MAVEN_SETTINGS_SECURITY_XML : ${MAVEN_SETTINGS_SECURITY_XML} - ${env.MAVEN_SETTINGS_SECURITY_XML}"
				        echo "MAVEN_SETTINGS_XML : ${MAVEN_SETTINGS_XML} - ${env.MAVEN_SETTINGS_XML}"

				        if (!vars.skipMavenSettings && env.MAVEN_SETTINGS_SECURITY_XML?.trim() && env.MAVEN_SETTINGS_XML?.trim()) {
				           vars.mavenGoals += " -Dsettings.security=${env.MAVEN_SETTINGS_SECURITY_XML} -s ${env.MAVEN_SETTINGS_XML}"
                }
