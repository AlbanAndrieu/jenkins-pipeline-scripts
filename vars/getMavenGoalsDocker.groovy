...
def call(Map vars, Closure body=null) {
    ...
    vars.JENKINS_CREDENTIALS = vars.get("JENKINS_CREDENTIALS", 'stash-jenkins').trim()
    ...
    if (!vars.skipDocker) {
        if (!DRY_RUN && !RELEASE) {
            echo "docker added"
            withCredentials([
              usernamePassword(credentialsId: vars.JENKINS_CREDENTIALS, passwordVariable: 'PASSWORD', usernameVariable: 'USER')
            ]) {
