...
def call(Map vars, Closure body=null) {
  ...
  //def DOCKER_REGISTRY = vars.get("DOCKER_REGISTRY", env.DOCKER_REGISTRY ?: "registry.nala.mobi").trim()
  ...
  def JENKINS_USER_HOME = vars.get("JENKINS_USER_HOME", env.JENKINS_USER_HOME ?: "/jenkins")
  ...
  vars.isNpmConfigPrefix = vars.get("isCredentialsMapping", false).toBoolean()
  ...
  if (vars.isVirtualHost == true) {
      ...
      DOCKER_OPTS_BASIC += ' -v /usr/local/sonar-build-wrapper:/usr/local/sonar-build-wrapper'
      DOCKER_OPTS_BASIC += ' -v /usr/local/sonar-runner/:/usr/local/sonar-runner/'
      ...
      if (vars.isLocalJenkinsUser == true) {
          DOCKER_OPTS_BASIC += DOCKER_OPTS_USER_ID
          DOCKER_OPTS_BASIC += ' -v ${JENKINS_USER_HOME}:/home/jenkins'
      } else (vars.isLocalMavenRepository == true) {
