...
def call(Map vars, Closure body=null) {
  ...
  vars.DOCKER_REGISTRY = vars.get('DOCKER_REGISTRY', env.DOCKER_REGISTRY ?: 'index.docker.io').toLowerCase().trim() // index.docker.io registry.hub.docker.com
  ...
  vars.DOCKER_REGISTRY_HUB = vars.get('DOCKER_REGISTRY_HUB', env.DOCKER_REGISTRY_HUB ?: 'index.docker.io').toLowerCase().trim() // index.docker.io/v1
  ...
  if (vars.DEBUG_RUN) {
    try {
      ...
      echo 'Warn: There was a problem. ' + exc
    }
  }
