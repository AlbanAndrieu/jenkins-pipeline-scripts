...
def call(Map vars, Closure body=null) {
  ...
  if (!vars.skipVersion) {
    try {
          ...
          if (data.dependencies.getClass() == java.util.ArrayList) {
            for (String item : data.dependencies) {
              ...
              if (item.repository.toString() == "@{vars.customRepoName}") {
                ...
                echo "Replacing version for : " + item.repository.name.toString() + " - for " + item.version.toString()
              } // if
            } // for
      ...
      echo "Warn: There was a problem with versioning helm umbrella charts " + exc.toString()
    } finally {
