...
def call(def DOCKER_BUILD_IMG = '', def DOCKER_TAG = 'latest') {
  if (isUnix()) {
    try {
      ...
      echo 'Warn: There was a problem removing image from local docker repo. ' + exc
    }
    try {
      sh '''#!/bin/bash -l
      ...
      echo 'Warn: There was a problem Cleaning local docker repo. ' + exc
        }
    try {
      sh '''#!/bin/bash -l
      ...
      echo 'Warn: There was a problem Cleaning local docker volumes. ' + exc
        }
  } // isUnix
}
