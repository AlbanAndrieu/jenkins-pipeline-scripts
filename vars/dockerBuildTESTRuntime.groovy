...
def call(Map vars, Closure body=null) {
    ...
    def DOCKER_TEST_RUNTIME_TAG = dockerTag(DOCKER_TAG)
    def DOCKER_TEST_RUNTIME_NAME = vars.get("DOCKER_TEST_RUNTIME_NAME", env.DOCKER_TEST_RUNTIME_NAME ?: "test")
    ...
    script {
        ...
        wrapInTEST() {
            ...
            sh(returnStdout: true, script: "echo ${DOCKER_TEST_RUNTIME_IMG} | cut -d'/' -f -1").trim()
            ...
            withRegistryWrapper() {
                def container = docker.build("${DOCKER_TEST_RUNTIME_IMG}", "${DOCKER_BUILD_ARGS} -f ${dockerFilePath}Dockerfile ${dockerTargetPath} ")
                //container.inside("--help") {
                //    sh 'java -version'
                //}

                echo 'Test container'
                def image = docker.image("${DOCKER_TEST_RUNTIME_IMG}").withRun("", "--version") {c ->
                ...
                if (!DRY_RUN && !skipPush) {
                    ...
                    if ( BRANCH_NAME ==~ /develop|master|master_.+|release\/.+/ ) {
                        echo "Push image ${DOCKER_TEST_RUNTIME_IMG} to DTR"
                        container.push()
                    } // if
                }
            } // withRegistryWrapper

            dockerFingerprintFrom dockerfile: "${dockerFilePath}Dockerfile", image: "${DOCKER_TEST_RUNTIME_IMG}"

        }  // wrapInTEST

    } // script

    return DOCKER_TEST_RUNTIME_TAG
