...
assert_role_exists() {
  ...
  if [[ ! -d "$EXPECTED_ROLE_DIR" ]]; then
    echo "Ansible role not found: $EXPECTED_ROLE_DIR"
    exit 1
  fi
  if [[ ! -f "$DEFAULT_MOLECULE_FILE" ]]; then
...
set_molecule_opts() {
  if ! $MOLECULE_QUIET; then
    MOLECULE_OPTS+=" --debug"
  fi
  if ! $MOLECULE_DESTROY; then
    if [[ $MOLECULE_TARGET == "test" ]]; then
...
set_python_executable
for ansible_role_name in "${ROLES_TO_TEST[@]}"; do
